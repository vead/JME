...
public class Voronoi {
     ...
     * @return
     */
    public List<GraphEdge> generateVoronoi(float[] xValuesIn, float[] yValuesIn,
    ...
    private void qsort(Site[] sites)
    {
        ...
        Collections.sort(listSites, new Comparator<Site>()
        {
            ...
            {
                Vector2f s1 = p1.coord, s2 = p2.coord;
                if (s1.y < s2.y)
                {
                    return (-1);
                }
    ...
    private void sortNode(float xValues[], float yValues[], int numPoints)
    {
        ...
        {
            sites[i] = new Site();
            sites[i].coord.set(xValues[i], yValues[i]);
            sites[i].sitenbr = i;
    ...
    private HalfEdge ELleftbnd(Vector2f p)
    {
        int i, bucket;
        HalfEdge he;
    ...
    private void pushGraphEdge(Site leftSite, Site rightSite, float x1, float y1, float x2, float y2)
    {
        ...
        newEdge.siteAnr = leftSite.sitenbr;
        newEdge.siteBnr = rightSite.sitenbr;
        
        newEdge.siteA = leftSite;
        newEdge.siteB = rightSite;
        
        newEdge.siteA.addAdjacentSite( rightSite );
        newEdge.siteB.addAdjacentSite( leftSite );
    }
    ...
    private boolean right_of(HalfEdge el, Vector2f p)
    {
        Edge e;
        Site topsite;
	...
	public Site getSiteByNbr(int number) {
		return sites[number];
	}

	public GraphEdge findBisectingGraphEdge( Site a, Site b ) {
