...
import no.jchief.phobos.support.Utils;
...
public class CameraAppState extends AbstractAppState implements Control, GameEventListener {
	...
	public void initialize(AppStateManager stateManager, Application app) {
		...
		GameEventSystem ges = pho.getGameEventSystem(); 
		ges.registerListener(InputEvent.class, this);

		//		this.cam = cam;

		// Disable first person camera
		pho.getFlyByCamera().setEnabled(false);
		pho.getInputManager().removeListener(pho.getFlyByCamera());
		...
		pho.getRootNode().addControl(this);
	}
	...
	public void update(final float tpf) {
		...
		pho.getCamera().setLocation(position);
		pho.getCamera().lookAt(center, new Vector3f(0,1,0));
